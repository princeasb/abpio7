...
public class SchoolAut0materDbMigrationService : ITransientDependency
{
    private readonly ILogger<SchoolAut0materDbMigrationService> _logger;
    private readonly ITenantRepository _tenantRepository;
    private readonly IDataSeeder _dataSeeder;
    private readonly ICurrentTenant _currentTenant;
    private readonly IUnitOfWorkManager _unitOfWorkManager;

    public SchoolAut0materDbMigrationService(
        ILogger<SchoolAut0materDbMigrationService> logger,
        ...
        CancellationToken cancellationToken)
    {
        using (var uow = _unitOfWorkManager.Begin(requiresNew: true, isTransactional: false))
        {
            ...
            await MigrateDatabaseAsync<StaffServiceDbContext>(cancellationToken);
            await MigrateDatabaseAsync<StoreServiceDbContext>(cancellationToken);
            await MigrateDatabaseAsync<PowerSchoolServiceDbContext>(cancellationToken);
